...
- e756f7f: Make `controls.attribution` render a real attribution control (schema-truthfulness U3). Previously the field validated but did nothing at runtime — the first of the "accepted but dead" fields the 0.4.0 truthfulness work closes.
...
- a8eca9b: Add a deprecation warning channel, and deprecate the interaction `action` fields (schema-truthfulness U8).
...
- 20c54e5: Implement `hover.highlight` (schema-truthfulness U5). The field validated and did nothing: hovering a feature now visibly highlights it.
...
- 6bb7c1f: Add the `raster-dem` source type (schema-truthfulness U6). `hillshade` has always been a valid layer type, but there was no valid source to feed it — the elevation-tile source MapLibre requires was missing from the union, so a hillshade map could not be expressed at all.
...
- 5be9676: Validation now delivers what it promised: real source positions, actionable type errors, and a warn-first typo channel.
