...
describe("warnings channel — unknown keys", () => {
  ...
  it("warns on a code-shaped field inside click.zoomToFeature", () => {
    // zoomToFeature is declarative (padding/maxZoom/duration only). A code- or
    // selector-shaped field is not on its shape, so it surfaces as an
    // unknown-key warning rather than being silently accepted.
    const result = YAMLParser.safeParseMapBlock(
      mapBlock(`  - id: p
    type: circle
    source: { type: geojson, url: "https://example.com/d.geojson" }
    ...
    const warning = result.warnings.find((w) =>
      w.path.endsWith("zoomToFeature.code")
    );
