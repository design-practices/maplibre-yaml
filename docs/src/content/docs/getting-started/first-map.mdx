---
title: Your First Map
description: Understanding the basics of maplibre-yaml configuration
---

import LiveMap from "../../../components/LiveMap.astro";
import { Aside } from "@astrojs/starlight/components";

This guide walks you through the fundamentals of maplibre-yaml configuration. By the end, you'll understand how to create maps, configure their appearance, and add basic layers.

## Map structure

Every maplibre-yaml configuration has this basic structure:

```yaml
type: map # Required: identifies this as a map block
id: my-map # Required: unique identifier

config: # Map settings
  center: [-74.006, 40.7128] # [longitude, latitude]
  zoom: 12 # Zoom level
  mapStyle: "https://demotiles.maplibre.org/style.json" # Style URL

layers: [] # Array of layer configurations

controls: {} # Optional: map controls
```

## The host page

The YAML on its own doesn't render anything — it needs a host page with the `<ml-map>` element, the MapLibre CSS, and the web component registered:

```html
<!DOCTYPE html>
<html>
  <head>
    <link
      href="https://unpkg.com/maplibre-gl/dist/maplibre-gl.css"
      rel="stylesheet"
    />
    <!-- Tell the browser where to find maplibre-gl -->
    <script type="importmap">
      { "imports": { "maplibre-gl": "https://esm.sh/maplibre-gl@^4" } }
    </script>
    <!-- Register the <ml-map> web component -->
    <script
      type="module"
      src="https://unpkg.com/@maplibre-yaml/core/register.js"
    ></script>
    <style>
      ml-map {
        display: block;
        height: 400px;
      }
    </style>
  </head>
  <body>
    <ml-map src="/my-map.yaml"></ml-map>
  </body>
</html>
```

<Aside type="caution">
  `<ml-map>` has no intrinsic size. If you don't give it `display: block` and
  an explicit height (via CSS or an inline `style` attribute), the map renders
  into a zero-height element and you'll see nothing — even though the YAML is
  valid.
</Aside>

## The config section

The `config` section defines the initial map view:

```yaml
config:
  center: [-74.006, 40.7128] # New York City
  zoom: 12 # Street level
  mapStyle: "https://demotiles.maplibre.org/style.json"

  # Optional settings
  pitch: 45 # Tilt angle (0-85)
  bearing: -17 # Rotation (-180 to 180)
  minZoom: 2 # Minimum zoom level
  maxZoom: 18 # Maximum zoom level
```

<LiveMap
  src="/configs/config-demo.yaml"
  height="350px"
  caption="Map with pitch and bearing applied"
/>

### Coordinates

Coordinates in maplibre-yaml use **[longitude, latitude]** order (GeoJSON standard), not latitude/longitude:

```yaml
# ✓ Correct: [longitude, latitude]
center: [-74.006, 40.7128]

# ✗ Wrong: [latitude, longitude]
center: [40.7128, -74.006]
```

<Aside type="tip">
  An easy way to remember: longitude comes first because it's the X axis
  (horizontal), latitude is Y (vertical).
</Aside>

### Map styles

The `mapStyle` property accepts any MapLibre-compatible style URL:

```yaml
# MapLibre demo tiles (free, good for testing)
mapStyle: "https://demotiles.maplibre.org/style.json"

# Self-hosted style
mapStyle: "https://your-server.com/styles/dark.json"

# MapTiler (requires API key)
mapStyle: "https://api.maptiler.com/maps/streets/style.json?key=YOUR_KEY"
```

## Adding a simple layer

Layers display data on your map. Here's a minimal circle layer:

```yaml
layers:
  - id: my-points # Unique layer ID
    type: circle # Layer type
    source: # Data source
      type: geojson
      data: # Inline GeoJSON
        type: FeatureCollection
        features:
          - type: Feature
            geometry:
              type: Point
              coordinates: [-74.006, 40.7128]
            properties:
              name: "New York"
    paint: # Visual styling
      circle-radius: 10
      circle-color: "#3b82f6"
```

<LiveMap
  src="/configs/simple-layer.yaml"
  height="350px"
  caption="Three US cities displayed as circle markers"
/>

## Map controls

Add navigation and other controls:

```yaml
controls:
  navigation: true # Zoom +/- buttons
  scale: true # Scale bar
  fullscreen: true # Fullscreen toggle
  geolocate: true # User location button
```

<LiveMap
  src="/configs/controls-demo.yaml"
  height="350px"
  caption="Map with navigation and scale controls"
/>

## Putting it together

Here's a complete example combining everything:

```yaml
type: map
id: complete-example
config:
  center: [-98, 39]
  zoom: 4
  mapStyle: "https://demotiles.maplibre.org/style.json"
  minZoom: 2
  maxZoom: 18

layers:
  - id: cities
    type: circle
    source:
      type: geojson
      data:
        type: FeatureCollection
        features:
          - type: Feature
            geometry:
              type: Point
              coordinates: [-74.006, 40.7128]
            properties:
              name: "New York"
              population: 8336817
          - type: Feature
            geometry:
              type: Point
              coordinates: [-118.243, 34.052]
            properties:
              name: "Los Angeles"
              population: 3979576
          - type: Feature
            geometry:
              type: Point
              coordinates: [-87.623, 41.881]
            properties:
              name: "Chicago"
              population: 2693976
    paint:
      circle-radius: 10
      circle-color: "#ef4444"
      circle-stroke-width: 2
      circle-stroke-color: "#ffffff"
    interactive:
      hover:
        cursor: pointer
      click:
        popup:
          - h3:
              - property: name
          - p:
              - str: "Population: "
              - property: population

controls:
  navigation: true
  scale: true
```

<LiveMap
  src="/configs/complete-example.yaml"
  height="400px"
  caption="Complete example with interactive popups and controls"
/>

## Next steps

Now that you understand the basics, learn about:

- [Layer types and styling](/guides/layers/)
- [Loading data from URLs](/guides/data-sources/)
- [Live data updates](/guides/live-data/)
