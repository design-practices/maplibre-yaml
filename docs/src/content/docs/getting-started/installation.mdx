---
title: Installation
description: How to install maplibre-yaml in your project
---

import { Tabs, TabItem } from "@astrojs/starlight/components";

## Supported maplibre-gl versions

maplibre-yaml works with maplibre-gl **v4 and v5** (v5 recommended for new
projects); the peer range is `^3.0.0 || ^4.0.0 || ^5.0.0`. The CDN examples
below pin `maplibre-gl@^5` because it is the current latest — pin whichever
major your project already uses.

## Package installation

<Tabs>
  <TabItem label="npm">
    ```bash npm install @maplibre-yaml/core maplibre-gl ```
  </TabItem>
  <TabItem label="pnpm">
    ```bash pnpm add @maplibre-yaml/core maplibre-gl ```
  </TabItem>
  <TabItem label="yarn">
    ```bash yarn add @maplibre-yaml/core maplibre-gl ```
  </TabItem>
</Tabs>

## Required CSS

MapLibre GL JS requires its CSS for proper rendering. Add it to your HTML or import it in your JavaScript:

<Tabs>
  <TabItem label="HTML">
    ```html
    <link
      href="https://unpkg.com/maplibre-gl/dist/maplibre-gl.css"
      rel="stylesheet"
    />
    ```
  </TabItem>
  <TabItem label="JavaScript">
    ```javascript import 'maplibre-gl/dist/maplibre-gl.css'; ```
  </TabItem>
</Tabs>

## Browser usage (CDN)

For quick prototypes or simple pages, you can use maplibre-yaml directly from a CDN — no build tools needed. An [import map](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/script/type/importmap) tells the browser where to find `maplibre-gl`, which is kept external (not bundled):

```html
<!DOCTYPE html>
<html>
  <head>
    <link
      href="https://unpkg.com/maplibre-gl/dist/maplibre-gl.css"
      rel="stylesheet"
    />
    <style>
      ml-map {
        display: block;
        height: 400px;
      }
    </style>
    <!-- Tell the browser where to find maplibre-gl -->
    <script type="importmap">
      { "imports": { "maplibre-gl": "https://esm.sh/maplibre-gl@^5" } }
    </script>
    <!-- Register the <ml-map> web component -->
    <script
      type="module"
      src="https://unpkg.com/@maplibre-yaml/core/register.js"
    ></script>
  </head>
  <body>
    <ml-map src="/map.yaml"></ml-map>
  </body>
</html>
```

To use the JavaScript API (`YAMLParser`, `MapRenderer`) from a CDN instead of the web component, import the package from esm.sh, which resolves its dependencies automatically:

```html
<script type="module">
  import { YAMLParser, MapRenderer } from "https://esm.sh/@maplibre-yaml/core";

  // Your map code here
</script>
```

## Web component registration

To use the `<ml-map>` web component, import the registration module:

```javascript
import "@maplibre-yaml/core/register";
```

Then use it in your HTML:

```html
<ml-map
  style="width: 100%; height: 400px;"
  config='{"type":"map","config":{"center":[-74,40],"zoom":10,"mapStyle":"https://demotiles.maplibre.org/style.json"},"layers":[]}'
>
</ml-map>
```

## TypeScript

maplibre-yaml is written in TypeScript and includes type definitions. No additional `@types` packages are needed.

```typescript
import { YAMLParser, MapRenderer } from "@maplibre-yaml/core";
import type { MapBlock } from "@maplibre-yaml/core";

// For a single `type: map` document:
const mapBlock: MapBlock = YAMLParser.parseMapBlock(yamlString);
```

## Verify installation

Create a simple test to verify everything is working:

```javascript
import { YAMLParser } from "@maplibre-yaml/core";

const yaml = `
type: map
id: test
config:
  center: [0, 0]
  zoom: 2
  mapStyle: "https://demotiles.maplibre.org/style.json"
layers: []
`;

const result = YAMLParser.safeParseMapBlock(yaml);

if (result.success) {
  console.log("✓ maplibre-yaml is working!");
  console.log("  Map ID:", result.data.id);
} else {
  for (const error of result.errors) {
    console.error(`✗ ${error.path}: ${error.message}`);
  }
}
```

## Next steps

Now that you have maplibre-yaml installed, let's [create your first map](/getting-started/quick-start/).
