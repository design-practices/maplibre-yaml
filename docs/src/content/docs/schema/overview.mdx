...
- **Source schemas**: GeoJSON, Vector, Raster, Raster DEM, Image, Video
...
```yaml no-validate
# ❌ Invalid - missing required 'center' and 'zoom'
type: map
id: my-map
config:
  mapStyle: "https://example.com/style.json"
...
Note that `mapStyle` itself is optional in the schema: the Astro integration can inherit it from a global `defaultMapStyle`. Standalone `<ml-map>` usage still needs it at render time.
