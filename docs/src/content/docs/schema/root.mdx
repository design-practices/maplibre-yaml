...
Define sources once at the map block level and reference them by name across multiple layers.
...
Layers reference a named source with its bare name (`source: earthquakeData`). The string path form `source: "#/sources/name"` is **not** resolved — it validates as a plain string but the source will never be found at render time. To share a full layer (including its source) across maps, use a named layer definition with a `$ref` object instead (see above).
...
When using `$ref` (as an object, e.g. `- $ref: "#/layers/layerName"` in a block's `layers` array), the format is:

- `#/layers/layerName` - Reference a named layer

References are resolved at parse time and replaced with the actual definition. Note that `$ref` must always be a YAML mapping with a `$ref` key — a plain string like `"#/layers/layerName"` is not treated as a reference. For sources, use block-level `sources:` definitions and reference them by bare name from `source:`.
