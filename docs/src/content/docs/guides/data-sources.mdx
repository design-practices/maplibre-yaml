...
<Aside type="tip">
  Use named sources when multiple layers share the same data (e.g., a fill layer and an outline layer from the same GeoJSON). Use inline sources for single-layer convenience.
</Aside>
