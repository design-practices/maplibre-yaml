...
description: Add interactive maps to Astro collection item posts using maplibre-yaml
---

import { Tabs, TabItem, Aside, Code } from "@astrojs/starlight/components";

This guide shows how to embed maps in Astro collection item posts using content collection schemas and map builder utilities. Whether you're creating a travel blog, restaurant reviews, or hiking guides, maplibre-yaml provides simple patterns for adding geographic context.

## Overview

The collection item integration approach uses three key concepts:

1. **Content Collection Schemas** - Pre-built Zod schemas that add location/region/route fields to your collection item frontmatter
...
<Aside type="tip">
  This guide assumes you're familiar with [Astro Content
  Collections](https://docs.astro.build/en/guides/content-collections/). If not,
...
```typescript
import { getCollectionItemWithLocationsSchema } from "@maplibre-yaml/astro";

const schema = getCollectionItemWithLocationsSchema();
```
...
```typescript
import { getCollectionItemWithRegionSchema } from "@maplibre-yaml/astro";

const schema = getCollectionItemWithRegionSchema();
```
...
```typescript
import { getCollectionItemWithRouteSchema } from "@maplibre-yaml/astro";

const schema = getCollectionItemWithRouteSchema();
```
...
For collection items with mixed content types, use the comprehensive schema:

```typescript
import { getCollectionItemWithGeoSchema } from "@maplibre-yaml/astro";

const schema = getCollectionItemWithGeoSchema({
...
<Aside type="note">
  When you omit `mapStyle` from map builder functions, the resulting
  configuration will have `mapStyle: undefined`. This works with the schema
  (which now allows optional `mapStyle`), but you'll need to ensure the style is
  resolved before rendering. The global configuration pattern above is the
  recommended approach for v0.1.0.
</Aside>

## Complete Blog Template Example

Here's a full example of a collection item post template with conditional map rendering:
