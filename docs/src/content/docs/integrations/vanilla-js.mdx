...
<Aside type="tip">
  That's it! No JavaScript code needed. The `<ml-map>` component automatically
  fetches, parses, and renders your YAML configuration.
</Aside>
...
<Aside type="tip">
  You can also use **both approaches together** - start with `<ml-map>` for basic rendering,
  then access the underlying `MapRenderer` via `mapElement.getRenderer()` when you need more control.
</Aside>
...
For quick prototypes or simple projects. To use the web component, provide `maplibre-gl` via an import map and load the register bundle:
...
To use the JavaScript API, import the package from esm.sh, which resolves its dependencies automatically:
...
<script type="module">
  import {
    YAMLParser,
    MapRenderer,
  } from "https://esm.sh/@maplibre-yaml/core";
  // Your code here
</script>
```
...
Two guides cover capabilities that apply directly to the vanilla-JS path:

- **[Format v2](/guides/format-v2/)** — the additive, opt-in `style:` / `runtime:`
  split. A v2 document parses to the same internal model as its v1 twin, so
  `YAMLParser` and `MapRenderer` consume it unchanged.
- **[Interactions & Eject](/guides/interactions/)** — how declarative
  interactions are authored, why a compiled `style.json` is inert on its own
  (the eject guarantee), and how `attachInteractions` wires behavior back onto
  any `maplibregl.Map`. This is also where the `emit` interaction and its
  trusted-policy gate are documented in full.
...
- [Format v2](/guides/format-v2/) - The `style:` / `runtime:` split
- [Interactions & Eject](/guides/interactions/) - Interactions, the eject guarantee, and `attachInteractions`
- [`examples/vanilla/eject-and-attach.html`](https://github.com/design-practices/maplibre-yaml/blob/main/examples/vanilla/eject-and-attach.html) - A runnable eject → `attachInteractions` demo: compile a document to a self-contained `style.json`, render it in a plain `new maplibregl.Map(...)`, then reattach the declarative interactions so a click opens a popup
