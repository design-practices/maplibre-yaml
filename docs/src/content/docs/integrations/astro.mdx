...
`buildFeatureMapConfig` reads from the local filesystem via `process.cwd()`. It is **build-time only** -- invoking it in a deployed SSR adapter context throws an error with guidance. Runtime resolution is planned for V2.
